...
- `ssh-add -t 4h` keeps the key loaded for four hours, after which the agent forgets it.
- `ssh-add -c` makes the agent demand confirmation (via `SSH_ASKPASS`) before every
  signing operation, so a rogue process can't silently use your key.
...
Agent forwarding doesn't copy your key, but it exposes your agent's socket on
the remote host. Anyone who can open that socket (root most obviously, and any
process running as your user) can ask your agent to sign authentication requests.
...
If you like the idea of a key that never has to exist anywhere beyond the machine in front of you, [PocketShell](https://pocketshell.io/#security), a browser-based SSH client, works the same way. Keys never sync at all: you enter one per host, it's stored encrypted in that one browser, and it's used in memory when you connect.
